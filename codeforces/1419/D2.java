...
public class Main {
	...
	static class D2SagesBirthdayHardVersion {
		...
		public D2SagesBirthdayHardVersion() {
		}
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			for(int i = 1; i<n; i += 2) {
				int target = i+1>=n ? -iinf : Math.min(ans[i-1], ans[i+1]);
				Integer x = left.lowerKey(target);
				if(x==null) {
					ArrayDeque<Integer> q = new ArrayDeque<>();
					for(var v: left.entrySet()) {
						for(int j = 0; j<v.getValue(); j++) {
							q.addLast(v.getKey());
						}
					}
					for(; i<n; i += 2) {
