...
public class Main {
	...
	static class BShortestCycle {
		...
		public BShortestCycle() {
		}

		private int bfs(int u) {
			int ret = iinf;
			ArrayDeque<Pair<Integer, Integer>> q = new ArrayDeque<>(n);
			HashMap<Integer, Integer> dist = new HashMap<>(1<<8), par = new HashMap<>(1<<8);
			dist.put(u, 0);
			...
			while(!q.isEmpty()) {
				...
				for(int x: graph[v.a]) {
					if(!dist.containsKey(x)) {
						dist.put(x, v.b+1);
						par.put(x, v.a);
						q.addLast(new Pair<>(x, v.b+1));
					}else if(par.getOrDefault(v.a, -1)!=x&&!par.getOrDefault(x, -1).equals(v.a)) {
						ret = Math.min(ret, dist.get(x)+dist.get(v.a)+1);
					}
				}
			}
			return ret;
		...
		public void solve(int kase, InputReader in, Output pw) {
			ArrayList<Integer>[] bit = new ArrayList[64];
			n = in.nextInt();
			for(int i = 0; i<64; i++) {
				bit[i] = new ArrayList<>();
			}
			...
			for(int i = 0; i<64; i++) {
				if(bit[i].size()==2) {
					int a = bit[i].get(0), b = bit[i].get(1);
					graph[a].add(b);
					graph[b].add(a);
				}
			}
