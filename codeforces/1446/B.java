...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			BCatchingCheaters solver = new BCatchingCheaters();
	...
	static class BCatchingCheaters {
		public BCatchingCheaters() {
		}

		public void solve(int kase, InputReader in, Output pw) {
			int n = in.nextInt(), m = in.nextInt();
			char[] a = in.next(n).toCharArray(), b = in.next(m).toCharArray();
			...
			for(int i = n-1; i>=0; i--) {
				for(int j = m-1; j>=0; j--) {
					...
					ans = Math.max(ans, dp[i][j] = Math.max(dp[i][j], 0));
				}
			}
