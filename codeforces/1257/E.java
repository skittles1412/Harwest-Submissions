...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			ETheContest solver = new ETheContest();
	...
	static class ETheContest {
		public ETheContest() {
		}

		public void solve(int kase, InputReader in, Output pw) {
			int x = in.nextInt(), y = in.nextInt(), z = in.nextInt(), n = x+y+z;
			int[] a = in.nextInt(x, o -> o-1), b = in.nextInt(y, o -> o-1), c = in.nextInt(z, o -> o-1);
			int[] acnt = new int[n], bcnt = new int[n], ccnt = new int[n];
			Arrays.fill(acnt, 1);
			Arrays.fill(bcnt, 1);
			Arrays.fill(ccnt, 1);
			for(int i = 0; i<x; i++) {
			...
			Utilities.Debug.dbg(dp2);
			int[] dp3 = new int[n+1];
			for(int i = n-1; i>=0; i--) {
				dp3[i] = Math.min(acnt[i]+dp3[i+1], dp2[i]);
			}
			pw.println(dp3[0]);
		}
