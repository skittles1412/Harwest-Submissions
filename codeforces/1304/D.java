...
public class Main {
	...
	static class DShortestAndLongestLIS {
		...
		public DShortestAndLongestLIS() {
		}

		public int[] solve(int[] arr) {
			int start = -1, prev = -2;
			int[] ret = new int[n], psum = new int[n+1];
			ret[0] = 1;
			for(int i = 1; i<n; i++) {
				if(local) {
					Utilities.Debug.dbg(process(ret, psum));
				}
				if(arr[i-1]==-1) {
					ret[i] = i+1;
				}else {
		...
		private int[] process(int[] arr, int[] psum) {
			int[] ret = arr.clone();
			int sum = 0;
			for(int i = 0; i<n; i++) {
				ret[i] += sum += psum[i];
