...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			DAbilityToConvert solver = new DAbilityToConvert();
	...
	static class DAbilityToConvert {
		public DAbilityToConvert() {
		}
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			int n = s.length();
			int end = n;
			BigInteger pow = BigInteger.ONE, ans = BigInteger.ZERO;
			for(; end>0; ) {
				int valid = end-1;
				for(int i = end-1; i>=0; i--) {
					...
					if(str.startsWith("0")) {
						continue;
					}else if(new BigInteger(str).compareTo(kbi)>=0) {
						break;
					}
					valid = i;
				}
				ans = ans.add(new BigInteger(s.substring(valid, end)).multiply(pow));
				pow = pow.multiply(kbi);
				end = valid;
			}
			pw.println(ans);
		}
