...
public class Main {
	...
	static class DDecreaseTheSumOfDigits {
		public DDecreaseTheSumOfDigits() {
		}

		public int sum(long x) {
			int ret = 0;
			while(x>0) {
				ret += x%10;
				x /= 10L;
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			int v = in.nextInt(), lastdigit = 0;
			while(sum(n)>v) {
				String s = new StringBuilder(String.valueOf(n)).reverse().toString();
				int i;
				for(i = lastdigit; s.charAt(i)=='0'; i++) ;
				lastdigit = i+1;
				long val;
				if(n<(long) 1e18) {
					val = Utilities.math.pow(10, i)*(10-(s.charAt(i)-'0'));
				}else {
					pw.println(new BigInteger("10000000000000000000").subtract(BigInteger.valueOf(n)));
					return;
				}
				ans += val;
				n += val;
				Utilities.Debug.dbg(i, val, n);
			}
			pw.println(ans);
		}
