...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			DStringDeletion solver = new DStringDeletion();
	...
	static class DStringDeletion {
		public DStringDeletion() {
		}
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			int start = 0, j = 0, ans = 0;
			while(j<n-1) {
				for(; j<n-1&&arr[j]!=arr[j+1]; j++) ;
				if(j!=n-1) {
					next.remove(j);
					start = next.ceiling(start);
					int x = arr[start];
					next.remove(start);
					for(; start<n&&arr[start]==x; start = next.higher(start)) {
						next.remove(start);
					}
					ans++;
				}
				j = Math.max(start, j+1);
//				Utilities.Debug.dbg(start, j);
			}
			ans += next.size() >> 1;
			pw.println(ans);
		}
