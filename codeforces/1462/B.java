...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			BLastYearsSubstring solver = new BLastYearsSubstring();
	...
	static class BLastYearsSubstring {
		final String s = "2020";

		public BLastYearsSubstring() {
		}

		public void solve(int kase, InputReader in, Output pw) {
			int n = in.nextInt();
			String cur = in.next(n);
			for(int i = 0; i<=4; i++) {
				if(cur.startsWith(s.substring(0, i))&&cur.endsWith(s.substring(i))) {
					pw.println("YES");
					return;
				}
			}
