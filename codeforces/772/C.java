...
public class Main {
	...
	static class CVulnerableKerbals {
		public CVulnerableKerbals() {
		}

		public long solve(int a, int b, int mod) {
			int gcd = Utilities.gcd(a, mod);
			a /= gcd;
			b /= gcd;
			mod /= gcd;
			return Utilities.math.modInverse(a, mod)*b%mod;
		}

		public void solve(int kase, InputReader in, Output pw) {
//			Utilities.Debug.dbg(solve(3, 7, 10));
			...
			int[] arr = new int[m+1];
			ArrayList<Integer>[] gcd = new ArrayList[m+1];
			for(int i = 0; i<=m; i++) {
				gcd[i] = new ArrayList<>();
			}
			for(int i = 0; i<m; i++) {
				if(valid[i]) {
					int cur = Utilities.gcd(i, m);
					arr[cur]++;
					gcd[cur].add(i);
				}
			}
			int[] dp = new int[m+1], next = new int[m+1];
			for(int i = m; i>0; i--) {
				dp[i] = 0;
				next[i] = -1;
