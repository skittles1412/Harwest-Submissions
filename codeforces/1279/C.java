...
public class Main {
	...
	static class CStackOfPresents {
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			for(int i = 0; i<n; i++) {
				arr.addLast(in.nextInt()-1);
			}
			HashSet<Integer> removed = new HashSet<>(n);
			long ans = 0;
			for(int i = 0; i<m; i++) {
//				Utilities.Debug.dbg(removed);
				int cur = in.nextInt()-1;
				if(removed.remove(cur)) {
					ans++;
				}else {
					int cnt = removed.size();
					while(true) {
