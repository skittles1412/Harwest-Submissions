...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			DEvenOddGame solver = new DEvenOddGame();
...
//			Utilities.Debug.dbg(val);
			long[] arr = new long[2];
			for(int i = 0; i<n; i++) {
				int x = val[0].size()-1, y = val[1].size()-1;
				if(val[0].get(x)>=val[1].get(y)) {
					int cur = val[0].remove(x);
					if(i%2==0) {
						arr[0] += cur;
					}
				}else {
					int cur = val[1].remove(y);
					if(i%2==1) {
						arr[1] += cur;
					}
				}
