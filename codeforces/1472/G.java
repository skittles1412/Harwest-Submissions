...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			GMovingToTheCapital solver = new GMovingToTheCapital();
	...
	static class GMovingToTheCapital {
		int n;
		int m;
		int[] arr;
		int[][] dp;
		...
		public GMovingToTheCapital() {
		}

		public int dp(int u, int left) {
			if(dp[u][left]!=-1) {
				return dp[u][left];
			}
			int ans = arr[u];
			for(int v: graph[u]) {
				if(arr[u]<arr[v]) {
					ans = Math.min(ans, dp(v, left));
				}else if(left==0) {
					ans = Math.min(ans, dp(v, left+1));
				}
			}
