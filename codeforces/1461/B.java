...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			BFindTheSpruce solver = new BFindTheSpruce();
	...
	static class BFindTheSpruce {
		public BFindTheSpruce() {
		}

		public void solve(int kase, InputReader in, Output pw) {
			int n = in.nextInt(), m = in.nextInt();
			int[][] arr = in.nextIntChar(n, o -> o=='.' ? 0 : 1);
			long ans = 0;
			for(int i = 0; i<n; i++) {
				for(int j = 0; j<m; j++) {
					ans += arr[i][j];
			...
			dp[0] = Utilities.clone(arr);
			for(int i = 1; i<=Math.min(n, m); i++) {
				dp[cur^1] = new int[n][m];
//				Utilities.Debug.dbg(dp);
				for(int j = 0; j<n-1; j++) {
					for(int k = 1; k<m-1; k++) {
						ans += dp[cur^1][j][k] = arr[j][k]&dp[cur][j+1][k-1]&dp[cur][j+1][k]&dp[cur][j+1][k+1];
					}
				}
				cur ^= 1;
			}
