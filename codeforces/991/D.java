...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			DBishwock solver = new DBishwock();
	...
	static class DBishwock {
		...
		public void solve(int kase, FastReader in, Output pw) {
			...
			int ans = 0, n = a.length(), prev = 0;
			for(int i = 0; i<n; i++) {
				int cur = getval(a.charAt(i))+getval(b.charAt(i));
				if(prev+cur>=3) {
					prev = prev+cur-3;
					ans++;
				}else {
