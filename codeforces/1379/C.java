...
public class Main {
	...
	static class CChoosingFlowers {
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			for(int i = 0; i<n; i++) {
				arr[i] = new long[] {flowers[i] = in.nextInt(), in.nextInt()};
			}
			Utilities.rsort(flowers);
			long[] psum = Utilities.psum(flowers);
			Utilities.reverse(flowers);
			long ans = 0;
			for(int i = 0; i<n; i++) {
				int val = Utilities.lowerBound(flowers, arr[i][1]), cnt = n-val, left = m-1;
				left -= (cnt = Math.min(left, cnt));
				long sum = arr[i][0]+psum[cnt];
				if(n-Utilities.lowerBound(flowers, arr[i][0])<=cnt) {
					sum -= arr[i][0];
					left++;
				}
				sum += arr[i][1]*(left);
