...
public class Main {
	static class TaskAdapter implements Runnable {
		...
		public void run() {
			...
			EAntonAndTree solver = new EAntonAndTree();
	...
	public static void main(String[] args) throws Exception {
		Thread thread = new Thread(null, new TaskAdapter(), "");
		thread.start();
		thread.join();
	}

	static class EAntonAndTree {
		int n;
		int comp;
		int[] arr;
		int[] conv;
		...
		public void solve(int kase, InputReader in, Output pw) {
			...
			}
			{
				...
				for(int i = 0; i<n; i++) {
					...
					for(int j: graph[i]) {
						if(conv[i]!=conv[j]) {
							ngraph[conv[i]].add(conv[j]);
						}
					}
				}
				graph = ngraph;
				arr = narr;
//				Utilities.Debug.dbg(conv);
